...
def call_with_retry(client_func, url, params=None, headers=None, max_retries=3):
    """Helper to call an API with retries for transient failures."""
    last_error = None
    for attempt in range(max_retries):
        try:
            resp = client_func(url, params=params, headers=headers, timeout=10.0)
    ...
    if last_error:
        raise last_error
    raise httpx.HTTPError("Max retries reached without response")
...
def geocode(query: str):
    ...
    try:
        with httpx.Client(headers={"User-Agent": USER_AGENT}) as client:
            resp = call_with_retry(client.get, "https://nominatim.openstreetmap.org/search", params=params)
...
def reverse_geocode(lat: float, lng: float):
    ...
    try:
        with httpx.Client(headers={"User-Agent": USER_AGENT}) as client:
            resp = call_with_retry(client.get, "https://nominatim.openstreetmap.org/reverse", params=params)
...
def get_weather(lat: float, lng: float):
    ...
    try:
        with httpx.Client() as client:
            resp = call_with_retry(client.get, "https://api.open-meteo.com/v1/forecast", params=params)
...
def get_soil_type(lat: float, lng: float, state: str = ""):
    ...
    try:
        with httpx.Client(headers={"User-Agent": USER_AGENT}) as client:
            resp = call_with_retry(client.get, url, params=params)
...
def get_crop_recommendations(soil_type: str, temp: float, rainfall: float):
    ...
    return {
        "recommended_crops": list(dict.fromkeys(recommended))[0:3],
        "profitable_crops": list(dict.fromkeys(profitable))[0:3]
    }
